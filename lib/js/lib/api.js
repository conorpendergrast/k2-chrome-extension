          ...
          $.ajax({
            ...
            $.ajax({
              ...
              $.ajax({
                ...
                item.reviews = data5;
                const reviewsByUser = _(data5).filter(r => r.user.login === currentUser);
                ...
                const userIsFinishedReviewing = (commented && !changesRequested) || approved;
                ...
                if (type === 'reviewed-by' && userIsFinishedReviewing) {
                  data.items = _(data.items).filter(i => i.id !== item.id);
                }
                done();
              });
            });
